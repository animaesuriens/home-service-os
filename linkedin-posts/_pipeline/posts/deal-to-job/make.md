...
HubSpot sends the won deal event with all customer data and project specs. Make parses this job data, extracting customer info, project scope, and contract details from the deal record.
...
If yes, Make skips that change order and moves to the next one — no duplicate job records created. If no, it processes the new addendum by creating a complete job record with full project data.
...
Make's visual canvas makes this flow crystal clear — you can see exactly how each deal branches into multiple job paths, with conditional logic preventing duplicates.
