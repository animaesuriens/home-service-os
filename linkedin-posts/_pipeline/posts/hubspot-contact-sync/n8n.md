...
Lead data hits your system from any marketing channel. n8n immediately triggers a 24-step flow that starts with a HubSpot contact search by ID. This catches most duplicates right away since each contact has a unique identifier.
...
When n8n finds an existing contact, it pulls the current properties and merges them with the fresh lead data. The automation loops through all contact engagements and deals, checking project types and marketing sources to build one complete profile.

If no contact exists anywhere, n8n generates a clean payload and creates a fresh HubSpot record with all the standardized fields your sales team expects.
...
n8n's node-based workflow makes the branching logic visual and intuitive. You can trace exactly how each lead flows through the search nodes, decision points, and merge operations.
